...
and "K8s" register as unrelated names. This vocabulary gives entities the same
canonical + alias shape the project and domain axes already have: a spelling
resolves to ONE identity instead of fragmenting across near-duplicate rows.
...
lowercase, then strip every character that is not `[a-z0-9]`. Every reader or
writer that resolves a spelling against this vocabulary must call this exact
function rather than re-implementing the rule, or the two could disagree about
whether two spellings name the same entity.
...
| `name` | `VARCHAR(255)` | NOT NULL. The verbatim canonical spelling |
| `normalized_key` | `TEXT` | NOT NULL, UNIQUE. `entity_normalize(name)`, maintained by a `BEFORE INSERT OR UPDATE` trigger rather than a generated/stored column — see the note below |
| `created_at` | `TIMESTAMPTZ` | default `now()` |
| `registered_by` | `VARCHAR(64)` | default `'system'` |

**`entity_vocab_aliases` — every other spelling, resolved in one hop.** Mirrors
`project_aliases`' junction shape: the alias string is kept verbatim,
`normalized_alias` is unique so two aliases can never collide once
normalized, and the FK (`entity_id`, `ON DELETE CASCADE`) points at the
canonical's IDENTITY, never at its name.

**Two triggers keep the two tables consistent, because no single CHECK can
see across them:**
- An alias must normalize to *exactly* its own canonical's `normalized_key` —
  enforced on write to `entity_vocab_aliases`. Since `normalized_key` is
  unique, this is what makes "an alias collides with a canonical" impossible
  except with its own parent, which is not a collision — it is what makes it
  an alias.
- A canonical must not normalize to a key some *other* entity is already
  registered as an alias for — enforced on write to `entity_vocabulary`.
...
either commits. Serializing writes to this vocabulary is a matter for the
ingress gate that becomes its write path in the later unit.
...
Per normalized-key group, the canonical pick is: highest count of
...
Every other spelling in the group becomes an alias of the pick.
