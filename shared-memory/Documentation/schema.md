...
| `rem_timing` | `JSONB` | Durable per-call REM latency summary (migration 019, decisions 570/571; comment re-issued by migration 037, fact:1621), written by `dream_telemetry.py`'s `record_llm_call`: `{service_ms,wall_ms,contention_ms,poll_ms,model,backend,batch_size,prompt_chars,completion_tokens,tok_s_wall,ts}`. `service_ms` (prompt_ms + predicted_ms from llama.cpp's own `timings` block) is pure inference = MODEL + HARDWARE, load-invariant; `wall_ms` is the client-observed call time; `contention_ms = max(0, wall_ms - service_ms)` is the CAPACITY signal (queueing behind a busy backend). `completion_tokens`/`tok_s_wall` are the OpenAI-compatible fallback for a backend that returns no llama.cpp `timings` block at all (external, non-llama.cpp providers): `tok_s_wall = completion_tokens / wall_s` is an EFFECTIVE rate that includes TTFT and network time, unlike `service_ms`'s pure-inference rate — present precisely where `service_ms`/`contention_ms` are NULL. `poll_ms` is `created_at` → REM pickup (daemon cadence). Survives `neo4j_outbox` row deletion on NREM consolidation, so it is the only durable record of per-call REM cost once a fact has finished dreaming. |
