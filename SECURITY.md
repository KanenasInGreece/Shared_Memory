...
- `vector-skill.py` uses FastMCP over **stdio** — no HTTP listener is opened, so the attack surface does not exist at all. When LM Studio launches it via `uv run --no-project mcp/vector-skill.py` — the connector's PEP 723 inline metadata pins `fastmcp` and `httpx` by exact version — the resolved environment uses Starlette 1.6.0 (patched) — the version `requirements-mcp.lock` pins and the one a real resolution produces, measured.
...
**Severity:** HIGH | **Files:** `coordinator.py` `auth_middleware`, `_router_match_path`, `require_unprotected_paths_are_plain_routes`; `hive_mind_proxy.py` `AsyncHiveMindProxy._route_guard`
...
The effect was that `GET /health/` — one trailing slash — skipped authentication *and* was not served by the health handler. The same held for `/health//`, the percent-encoded `/health%2f`, and `/pool/status/`. `GET /pool%2fstatus` was likewise exempted from authentication, because the middleware read the decoded path; it did not reach a backend, because `_route_guard` refuses any request whose *decoded* path names a known route but which the router did not resolve to that route's handler — a containment this fix now pins with a test, since it is what the auth-off default relies on for that spelling.
...
1. **Route-owned refusal.** `_route_guard` — which already refuses a wrong-method or unregistered call under the framework's reserved path prefixes, before any LLM dispatch decision — now also refuses a trailing-slash near-miss of a route the gateway *owns*. `/health` and `/pool/status` sit outside the reserved prefixes, which is why they were the two names the existing guard did not already cover (`/memory/search/` and `/memory/telemetry/` were closed by it all along). The refusal is the guard's existing 404 in its existing voice, and it says explicitly that the request was not forwarded to any LLM backend. It runs *after* the reserved-prefix refusal, so the reply to an unregistered `/memory/*` or `/admin/*` path is unchanged and does not distinguish a registered route name from an unregistered one.
2. **The exemption compares the string the router compares.** `coordinator.py` `_router_match_path` reads `request.rel_url.path_safe` — the exact value aiohttp's `Resource.resolve` passes to `PlainResource._match`, whose test is `self._path == path`. Membership in the exempt set is therefore the router's own admission test, evaluated on the router's own input, and the middleware cannot form a second opinion about what `/health` means. Nothing is normalised and nothing is decoded on the granting side; normalisation is used only to *refuse*. A startup assertion (`require_unprotected_paths_are_plain_routes`) refuses to launch if an exempt entry is not the canonical of a *static* route, which is the precondition that equivalence rests on: for a dynamic resource `canonical` is many-to-one, so an exact compare would stop modelling the router's match and could exempt a whole pattern family.

Not using `request.path` is the load-bearing detail rather than a stylistic one. `request.path` is fully percent-decoded, so `/pool%2fstatus` reads there as the complete exempt key `/pool/status`, while the router — matching on `path_safe`, where `%2F` stays encoded — resolves it to the catch-all. An exemption keyed on the decoded path grants authentication-free passage to a request the router is about to hand to the LLM proxy. `path_safe` closes that by construction.
...
**Scope of the refusal.** The near-miss refusal normalises with `rstrip("/")` and so covers *trailing-slash* spellings only: `/health/`, `/health//`, `/health%2f`, `/pool/status/`. It is not a general path canonicaliser. `//health`, `/pool//status` and `/health/.` are not near-misses to it — with authentication configured they are a plain 401, and on an auth-off install they reach the LLM backend exactly as any other unregistered path does, which is the auth-off posture rather than a residue of this defect.
...
| Write-Cypher guard bypass via tabs/comments (as originally framed) — measured bypass was via **double whitespace**, not tabs/comments | **Corrected 2026-08-27 (v0.9.69) — was NOT a false positive; this call was wrong.** Measured: the old regex's `SET\s` alternative sits inside `\b(...)\b`, so it needs a word-boundary immediately after the ONE whitespace character `\s` consumes. A query with a DOUBLE space after `SET` (`MATCH (n) SET  n.prop = 1`) places that boundary between two whitespace characters — never a word/non-word transition, so never a boundary — and the alternative fails to match at all; the guard's regex layer missed the query entirely. **What held:** the Neo4j session's `default_access_mode="READ"` is a second, independent layer enforced at the driver level, so the bypass query still could not write — only the regex layer failed, not both, and no write occurred in the reproduction. **Fix:** the keyword-boundary regex `\b(CREATE\|DELETE\|DETACH\s+DELETE\|SET\|REMOVE\|MERGE\|CALL\|LOAD\s+CSV\|DROP)\b` — dropping the `\s` after `SET` so the trailing `\b` alone bounds the keyword regardless of how much whitespace follows — covered by `tests/test_graph_route_guard.py` (table-driven: single space, double space, tab, newline, lowercase, and the property-name cases `n.settings`/`n.asset` the original `\s` guard existed to let through, which still pass). |
