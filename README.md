...
> **Grade: Strong Partial (v0.3.5).** Decisions trace fully to human (`WAS_ATTRIBUTED_TO`), AI agent (`WAS_ASSISTED_BY`), and project (`PROJECT_OF`). `HAD_OUTCOME` dated edges close the forward trace: decision → outcome → rating + notes (the Why-To loop). Community summaries link back to source facts via `source_pg_ids`. The optional `source_ref` key (e.g. `"design-doc.pdf#p12"`, `"meeting.mp4@00:04:32"`) propagates to `Fact.source_ref` in Neo4j. Agent source attribution is server-verified (v0.3.5). **Gaps remaining:** `source_ref` is not enforced — agents supply it when they can; no back-edge from a raw `Fact` to the `Decision` it influenced; read events are not audited.
