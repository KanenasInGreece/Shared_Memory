...
![Claude Code](https://img.shields.io/badge/Claude_Code-CLI-blue)
...
- **Claude Code** — uses a CLI bridge (`memory_bridge.py`) to search, save, and query the graph. No MCP server required; the CLI is the interface.
- **Gemini CLI** — uses the same CLI bridge, packaged as a skill (`/activate shared-memory`). The same scripts, the same backend, a different invocation surface.
...
What makes the three tools a unified memory system rather than three separate stores is the embedding model. Every vector in the system — saved by Claude Code, saved by Gemini CLI, saved by LM Studio, re-embedded by the consolidation daemon — was generated by the same BGE-M3 instance through the same gateway. The coordinate system is shared. Cosine similarity between a vector Claude Code saved last Tuesday and a query Gemini CLI is making right now is a genuine semantic comparison.
...
│                          AGENT LAYER                                     │
│                                                                          │
│   Claude Code (CLI)        Gemini CLI (skill)       LM Studio (MCP)      │
...
|---|---|---|---|
| **Claude Code** | CLI only | `shared-memory/scripts/memory_bridge.py` | `pg_notify` on save |
...
### CLI usage (Claude Code and Gemini CLI)
...
uv run --with httpx --with psycopg2-binary --with neo4j \
  ...
  '{"source":"claude_code","entities":["hive_mind_proxy","BGE-M3"]}'
