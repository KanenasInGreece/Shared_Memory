...
- **Human-asserted entity anchoring (v0.8.60):** entities have run full circle to a clean, well-defined role — anchored strictly to Facts on ingress (`(f:Fact)-[:MENTIONS]->(e:Entity)`), while decisions and retrospectives traverse canonical fixpoint paths, saving REM cycle time and LLM effort.
...
### CPU or GPU — both work, and the difference is time

**As packaged, both encoders run on CPU.** That is deliberate: it means the stack comes up on any machine with no GPU, no driver and no vendor branch. It costs time, and the cost is worth knowing before you decide.

Measured on the reference rig (AMD Ryzen 9 5900X, both containers CPU-only at 4 threads, BGE-M3 and BGE-Reranker-v2-m3 at Q8_0):

| Work | Cost on CPU |
|---|---|
| Embed one 8,000-char record | ~20 s |
| Rerank a typical 20-candidate search (73k chars) | **~64 s** |
| Rerank a worst-case 20-candidate search (129k chars) | **~146 s** |
| Rerank the same typical search, each document capped to 2,000 chars | ~30 s |

Two things follow. First, **the reranking timeout must be derived from the payload, never fixed** — the framework does this (`rerank_ceiling`, `RERANK_MIN_CHARS_S`), and a constant timeout is how reranking silently stops happening at all. Second, **throwing CPU threads at it barely helps**: on the same payload, 4 threads took 63.8 s, 10 threads 36.7 s and 20 threads 32.3 s — five times the threads for twice the speed, because these models saturate memory bandwidth (and, on multi-die CPUs, the die-to-die fabric) long before they saturate cores.
...
>
> **And do not point `EMBEDDER_URL` at the reranker.** The ports are one digit apart and the failure is silent: asked to embed, the reranker answers HTTP 200 with a 1024-dimension vector — the right shape, essentially all zeros. It passes a dimension check and a null check while carrying no meaning. The embedder, asked to rerank, correctly refuses with HTTP 501.
...
### Entity Architecture — Full Circle: Human-Asserted Fact Entity Anchoring & Containment (v0.8.60)

The original role of entities in the framework has **run full circle**: from early raw-text LLM extraction and heavy background alias sweeps (`alias_writer.py`, `alias_graph.py`, `entity_resolution_eval.py`) to a clean, well-defined, and contained architecture:

1. **Fact-Anchor Exclusivity (E1 & E3):** `:Entity` nodes in Neo4j exist strictly on Facts via `(f:Fact)-[:MENTIONS]->(e:Entity)`. On Fact ingress (`/memory/save`), human-asserted entity strings are registered in Postgres `entity_registry` (`030_entity_registry.sql`), keeping entity vocabulary vetted and precise.
2. **Zero Judgement Entity Edges (E2 & E5):** `Decision` and `Retrospective` nodes carry **0 entity edges** in Neo4j. Free-prose graph decision-extras (`CONSIDERED`, `REJECTED`, `UNDER_CONDITIONS`, `PRODUCES_INSIGHT`) targeting `:Entity` nodes were retired. Decision options live cleanly in Postgres `decision_alternatives` with pgvector embeddings.
3. **REM De-scoping (E4):** REM performs 0 entity minting and 0 entity `MENTIONS` edge writing (`if label == ONT.entity: continue`). Heavy background alias sweeps and re-embedding sweeps are retired.
4. **Canonical Fixpoint Traversal:** Non-Fact nodes (`Decision`, `Retrospective`, `CommunitySummary`) access entity context symmetrically by walking the fixpoint path `GROUNDED_IN | INFORMED_BY | HAD_OUTCOME` to live Facts.
5. **Operational Efficiency:** Eliminating LLM entity extraction and decision-extra graph writing during REM dreaming dramatically reduces REM cycle time and GPU/LLM token consumption while preserving 100% graph precision.
...
| **Entity alias layer + framework env architecture** | `ALIASES` Entity↔Entity soft edges (never a hard merge; always reversible). `alias_graph.py` runs `gds.wcc` to stamp `Entity.alias_component`; NREM groups by alias component so synonym variants fold as one cluster keyed on lexicographic-min canonical name. Search surfaces aliases in `graph_context`. `/memory/telemetry` carries `entity_graph` section (`alias_edges`, `alias_covered_entities`, `top_hubs`, …). Offline calibration harness `entity_resolution_eval.py`. Framework env moved to `shared-memory/.env`; `${VAR}`-parametrized compose; `install_framework.sh` first-install script. **Neo4j GDS plugin required.** | ✅ Done (v0.6.0) |
| **Entity Culling, Containment, & Canonical Traversal** | Entities ran full circle: Fact-anchor exclusivity (E1), zero judgement entity edges (E2), Postgres `entity_registry` (E3, migration 030), REM entity de-scoping (E4), decision-extras retired (E5). Canonical fixpoint entity traversal over `GROUNDED_IN | INFORMED_BY | HAD_OUTCOME` to live Facts. Retired `alias_writer.py`, `alias_graph.py`, `entity_resolution_eval.py`. Purged 3,695 non-Fact entity edges and 1,049 orphan entities on live graph; saved REM cycle time and LLM effort. | ✅ Done (v0.8.60) |
