...
### Security
- Neo4j image pin `5.26.30-community` → `5.26.31-community` on the same 5.26 LTS. The query log escapes the unicode in CVE-2026-1337, and Jackson is 2.22.2 (CVE-2026-68497, CVE-2026-19032, CVE-2026-83557). Cypher and the store format are unchanged. The running container moves when `reconcile_stack.sh` recreates it.

### Changed
- GPU encoder services pass `--flash-attn on`. If that server exits with a status other than 0, SIGINT, or SIGTERM, `encoder_gpu_or_cpu.sh` restarts it in the same container on CPU, with the flag removed and layer offload 0. The CPU encoder services do not set the flag.
- The README names the measured layouts and their latencies: Main workstation configurations 1–3, glxvm (8 GB RX 580, both encoders, 20 GB RAM, online LLM), and d9400 (embedder on a 4 GB card, reranker on CPU).
...
- **`requirements.lock`** pins the full runtime dependency tree (81 packages) to the
  exact tested versions, with hashes, generated by `uv pip compile --generate-hashes`
  and audited clean with `pip-audit` at generation time. Reproduce the tested
  environment with `uv venv && uv pip sync requirements.lock`. `requirements.txt`
  keeps the floors and gains the two runtime dependencies the daemons import but
  never declared (`json-repair`, `numpy`).
- **`requirements-gateway.lock`** (declared in `requirements-gateway.txt`) pins the
  gateway process's deliberately *restricted* dependency set — narrower than the full
  tree because the gateway must not carry `psycopg2` (its code paths are required to
  survive that module being absent; `tests/test_nrem_gate_import_purity.py` guards
  this). The shipped systemd unit now runs from this lock, so a fresh install's
  service gets the tested versions and `git pull` advances them with the code. A new
  test guard asserts the gateway lock can never gain `psycopg2` — proven against the
  known-broken artefact before landing.
- **AGENTS.md Phase 7 makes pinning the agent-installer default**, stated to the user
  when acting, with the unpinned latest-resolution form offered as the operator's
  choice.

### Fixed — install-surface staleness (every defect reproduced first)

- `install_framework.sh` printed a doubled compose path that fails as pasted, and
  pointed token minting at `generate_tokens.py` directly instead of the documented
  `bootstrap_tokens.sh` flow; its next-steps now match the Quick Start order,
  including `init_db.sh`.
- `preflight.sh`'s success line omitted `--env-file shared-memory/.env`, so the
  suggested compose command would miss the framework env; its RAM warning now defers
  to the measured configurations in README §3 instead of a stale estimate.
- `CONTRIBUTING.md`'s documented test command failed collection with 19 errors (four
  missing `--with` dependencies) — fixed and re-proven. Its invariants list still
  claimed entities gate Tier 3 consolidation; corrected to the current contract:
  consolidation folds on `(project, domain)`, entities are optional and
  operator-explicit.
- `AGENTS.md` Phase 2's hardware note aligned with README §3 (a GPU is optional).

### Also riding this release — compose and documentation work since 0.9.14

- **One yaml, either encoder pair (#260, #261):** the shipped compose file carries
  both the CPU encoders (`CPU_ENCODER_REPLICAS`, default 1) and the Vulkan GPU pair
  (`GPU_ENCODER_REPLICAS`, default 0); the choice is two `.env` lines, and both-zero
  means the encoders are hosted outside the stack. The GPU overlay file remains as
  marked legacy.
- **README (#259, #262):** measured GPU-encoder story, model-mixing knobs and the
  tested no-local-LLM path; system requirements rewritten as three measured example
  configurations (CPU-only + online LLM · a ~4 GB card for the encoders ·
  everything-local at 16 GB RAM + one 12 GB card).
- **AGENTS.md (#263):** `/health` has two shapes on an auth-configured install —
  triage reads the authenticated payload; the anonymous shape is liveness only.
