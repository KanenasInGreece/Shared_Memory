...
`registry_unavailable` (503) — the registry itself could not be consulted (a lookup error, not an
unknown name); retry the save, it is not refused, only deferred.
...
**Re-saving identical content never moves its axes.** Once a record exists, only `supersedes` or a
...
project and domain (from what it rests on or judges — never entities, which a judgement carries none of).
...
uv run --with httpx \
  ...
  '{"source":"claude_code","entities":["coordinator","OutboxPattern"]}'
...
# → {"status":"ok","api_version":4,"version":"0.9.100"}
...
- **Version:** `python ~/.gemini/skills/shared-memory/scripts/memory_bridge.py --version` → `{"version": "0.9.100", "api_version": 4, "tool": "shared-memory-framework"}`
