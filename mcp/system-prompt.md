...
- **`graph_query`** — read-only Cypher for multi-hop paths and provenance chains the automatic expansion did not reach. This tool requires `full` or `admin` (`read` tokens receive 403). Reading stays open to a `read` role: `hybrid_search_and_rerank`, `record_lineage`, `check_memory_health`, `memory_telemetry`. The CLI's named `query` templates have no tool here — write the Cypher, or ask the operator. `CREATE`, `DELETE`, `DETACH DELETE`, `SET`, `REMOVE`, `MERGE`, `CALL`, `LOAD CSV`, `DROP` are blocked by the gateway keyword guard (write-Cypher is blocked for everyone). A query Neo4j itself rejects returns 400 `cypher_rejected` with the driver's message — read it and fix the Cypher; re-sending it unchanged will never succeed. A 500 `query failed` is the gateway or Neo4j, and retrying is reasonable.
