...
- Graph queries: `POST /memory/graph` requires a token with `full` or `admin` role (`read` tokens receive 403) and is read-only Cypher — enforced by a keyword guard (blocks `CREATE`, `DELETE`, `DETACH DELETE`, `SET`, `MERGE`, `CALL`, `LOAD CSV`, `DROP`; `default_access_mode="READ"` is a driver routing hint, not a server-enforced security boundary). Write-Cypher is blocked for everyone. A read-only query the DATABASE refuses — syntax error, unknown function, type error — comes back as 400 `cypher_rejected` carrying Neo4j's own message: your query to fix, never a retry. Only a real fault is a 500
