...
- Graph queries: `POST /memory/graph` requires a token with `full` or `admin` role (`read` tokens receive 403) and is read-only Cypher — enforced by a keyword guard (blocks `CREATE`, `DELETE`, `DETACH DELETE`, `SET`, `REMOVE`, `MERGE`, `CALL`, `LOAD CSV`, `DROP`; `default_access_mode="READ"` is a driver routing hint, not a server-enforced security boundary). Write-Cypher is blocked for everyone. A read-only query the DATABASE refuses — syntax error, unknown function, type error — comes back as 400 `cypher_rejected` carrying Neo4j's own message: your query to fix, never a retry. Only a real fault is a 500
...
3. **Your ROLE decides which writes succeed, and a refusal is an answer.** Every identity is
   registered with a role: `search`, `lineage`/`status`, and `telemetry` remain for `read`, while
   `graph_query` and mutation tools (`save_artifact`, `save_decision`, `save_retrospective` and
   `supersede`) answer with an honest 403. That 403 is the system working — do not retry it, do
   not route around it, and say plainly that the record was not saved rather than reporting a save
   that did not happen. Where writes ARE permitted, the same discipline as everywhere: propose the
   record and confirm with the operator before saving a decision, never auto-decide.
...
2. **`rag-orchestrator` → `graph_query`** — only if step 1 returned insufficient graph depth and your token has `full` or `admin` role (`read` tokens receive 403). Multi-hop paths and provenance chains that the automatic expansion did not reach: read-only Cypher, through the gateway.
...
A search result for a summary or insight may carry `stale_sources: [{old, superseded_by}]` — `old` may be a fact, a decision, or a retrospective that has since been **superseded** (corrected/retracted), so that part of the narrative may be stale. Use the qualified ref. Fetch the successor (`superseded_by`) and compare before relying on it; a null successor means the source was retracted with no replacement. Superseded records are explicit (never inferred from similarity), kept for provenance, and excluded from search and consolidation.
...
Facts and decisions saved by one agent (Claude Code, Antigravity CLI, Grok) are retrievable by this model as soon as the search is run. Tier-3 is ranked with Tier-1. No tier has a reserved slot. `ranked: false` omits Tier-3. A community summary is a synthesised narrative across all agents' contributions. An insight is a decision-validated principle spanning multiple projects, scored on the same scale as the thematic summary.

```
hybrid_search_and_rerank("coordinator deadlock prevention", 5)
→ {"tier": "community_summary", "content": "The coordinator uses sorted per-entity locks..."}
→ {"tier": "fact", "content": "...", "graph_context": [{"rel_type":"WAS_ATTRIBUTED_TO","name":"Xenofon",...}]}
```
...
- **`memory_telemetry`** — pull the gateway's operational snapshot (`GET /memory/telemetry`): **the numbers**, each with the limit stated next to it. `encoders` (per-call embed/rerank latency — p50/p95/max, calls, errors), `gateway` (requests, status split, latency percentiles, in-flight, load-shed 503s), `outbox` (pending/applied/**`failed`, always present even at zero**, oldest ages, apply-latency percentiles, drain rate), `postgres` and `neo4j` (pool gauges, wait and query percentiles, `cypher_rejected_total` vs `tx_failures_total`), `rem` (dead-lettered, failing, passed-over, throughput), `nrem` (consolidation-cycle counts, served from the gateway's 60-second refresher with an `as_of` stamp because the insight walk is unbounded), `registry` (project/domain/alias row counts and the ingress refusal counters), `llm` (pool, affinity, routing, token usage, latency, faults), `clients.versions_seen`, plus `breakdown` (record types, agents, sources, `projects` and `domains`), `spine`, `consolidation`, `compliance` and the **`entity_graph`** section (`entities_total`, `singleton_entities` (mentioned by exactly one fact — a fragmentation proxy), `orphan_entities` (**truly dangling — degree-0, no edge of any kind**; an entity reached only by REM typed edges is *not* an orphan and is reported separately as `unmentioned_entities`), `genuinely_referenced_entities` and `top_hubs`). The **`inference_busy`** signal (tri-state `"busy"|"idle"|"unknown"`, also top-level on `GET /health`) is the `nvtop` GPU-busy check the daemons gate on, where `"unknown"` (nvtop absent / `SLOT_AWARE=0`, or the probe disabled itself after repeated hangs — see `gpu_probe`) is never reported as a false `"idle"`. `generated_at` is when the payload was built (it is briefly cached) and `timestamp` when it was served. Read-only; no direct DB access. The key-by-key contract is `Documentation/telemetry-contract.md`.
- **`check_memory_health`** — **is it usable right now**: `status` (`ok` | `degraded` | `down`) derived from `dependencies` — one enum per dependency (postgres, neo4j, embedder, reranker, llm_pool, rem_daemon, nrem_daemon, outbox, registry), each with a `reason` when it is not `ok` — plus `warnings`, the list of limits that have been crossed as `{key, limit, observed, unit}`. The NUMBER behind a warning is on `memory_telemetry`. ⛔ **HTTP 503 means one thing: the embedder or the reranker is down**, so a save cannot produce a vector; every other verdict is served 200 with the enum in the body. `consolidation.stalled` means an eligible backlog exists but nothing has folded — investigate.
...
- **`graph_query`** — read-only Cypher for multi-hop paths and provenance chains the automatic expansion did not reach. `graph` and named CLI `query` templates require `full` or `admin` (`read` tokens receive 403); `search`, `lineage`/`status`, and `telemetry` remain for `read`. `CREATE`, `DELETE`, `DETACH DELETE`, `SET`, `REMOVE`, `MERGE`, `CALL`, `LOAD CSV`, `DROP` are blocked by the gateway keyword guard (write-Cypher is blocked for everyone). A query Neo4j itself rejects returns 400 `cypher_rejected` with the driver's message — read it and fix the Cypher; re-sending it unchanged will never succeed. A 500 `query failed` is the gateway or Neo4j, and retrying is reasonable.
- **`archive_reasoning_trace`** — the tool posts `type` `reasoning_trace`. Ingress returns `unknown_type`. Do not retry. Save the conclusion as a fact.
