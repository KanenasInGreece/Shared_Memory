...
- **`graph_query`** — read-only Cypher for multi-hop paths and provenance chains the automatic expansion did not reach. Requires a token with `full` or `admin` role (`read` tokens receive 403). `CREATE`, `DELETE`, `SET`, `MERGE`, `CALL`, `LOAD CSV`, `DROP` are blocked by the gateway keyword guard (write-Cypher is blocked for everyone). A query Neo4j itself rejects returns 400 `cypher_rejected` with the driver's message — read it and fix the Cypher; re-sending it unchanged will never succeed. A 500 `query failed` is the gateway or Neo4j, and retrying is reasonable.
